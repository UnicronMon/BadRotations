...
---@param from? string
actionList.utility = function(from)
...
---@param from? string
actionList.interrupts = function(from)
...
---@param from? string
actionList.precombat = function(from)
...
actionList.def = function(from)
    ...
    if cast.able.rake() and isStealthed() then
    ...
    if talent.adaptiveSwarm and not (bt.need and bt.triggers == 2) and (not talent.unbridledSwarm or var.swipeTargets == 1) then
        for i = 1, #var.enemies.yards5f do
            local thisUnit = var.enemies.yards5f[i]
    ...
    if talent.adaptiveSwarm and debuff.adaptiveSwarmDamage.stack() < 3 and talent.unbridledSwarm and var.swipeTargets > 1 and not (bt.need and bt.triggers == 2) then
        for i = 1, #var.enemies.yards5f do
            local thisUnit = var.enemies.yards5f[i]
    ...
    if actionList.cooldown('def - 13') then return true end
    ...
    if cast.able.feralFrenzy(var.maxTTDUnit) and ((talent.direFixation and debuff.direFixation.exists(var.maxTTDUnit)) or not talent.direFixation or var.swipeTargets > 1) and (var.comboPoints < 2 or (var.comboPoints < 3 and var.bsIncUp))
    then
    ...
    if cast.able.ferociousBite(var.maxTTDUnit) and buff.apexPredatorsCraving.exists() and ((var.swipeTargets == 1 or not talent.primalWrath or not buff.sabertooth.exists()) and not (bt.need and bt.triggers == 2)) then
    ...
    if var.bsIncUp then
        ...
        if actionList.berserk('def - 16') then return true end
    end
    -- 17 wait,sec=combo_points=5,if=combo_points=4&buff.predator_revealed.react&energy.deficit>40&spell_targets.swipe_cat=1
    if (var.comboPoints == 4 and buff.predatorRevealed.react() and var.energyDeficit > 40 and var.swipeTargets == 1 and power.comboPoints.ttm() < br.player.gcdMax) then
    ...
    if (var.comboPoints == 4 or var.comboPoints == 5) and not (buff.bloodtalons.stack() <= 1 and bt.triggers == 2 and var.swipeTargets == 1) then
        --ui.debug("Calling Action List [finisher] from [def] - 18")
        if actionList.finisher('def - 18') then return true end
    end
    ...
    if bt.need and not var.bsIncUp and var.comboPoints < 5 then
        ...
        if actionList.bloodtalons('def - 19') then return true end
    end
...
---@param from? string
actionList.aoe_builder = function(from)
    ...
    if cast.able.shadowmeld() and (debuff.rake.refresh() or debuff.rake.applied() < 1.4) and not isStealthed() and not buff.apexPredatorsCraving.exists()
    then
...
---@param from? string
actionList.berserk = function(from)
    ...
    if cast.able.ferociousBite(var.maxTTDUnit) and var.comboPoints == 5 and debuff.rip.remain(var.maxTTDUnit) > 8 and var.ui.berserkBiteweave and var.swipeTargets > 1 then
    ...
    if var.comboPoints == 5 and not (buff.overflowingPower.stack() <= 1 and bt.triggers == 2 and buff.bloodtalons.stack() <= 1) then
        --ui.debug("Calling Action List [finsiher] from [berserk] - 2")
        if actionList.finisher('berserk') then
            return true
        end
    end 
    ...
    if cast.able.prowl() and var.ui.prowlMode and not (bt.rake and bt.triggers == 2)
        ...
        and not isStealthed() and (talent.feralFrenzy and cd.feralFrenzy.remain() < 44) and not buff.apexPredatorsCraving.exists()
    then
    ...
    if cast.able.shadowmeld() and not (bt.rake and bt.triggers == 2) and cast.able.rake() and not buff.suddenAmbush.exists()
        ...
        and not isStealthed() and not buff.apexPredatorsCraving.exists()
    then
        if cast.shadowmeld() then
    ...
    if cast.able.shred(var.maxTTDUnit) and (bt.triggers == 2 or (talent.direFixation and not debuff.direFixation.exists(var.maxTTDUnit))) and not bt.shred then
        if cast.shred(var.maxTTDUnit) then
    ...
    if talent.lunarInspiration and cast.able.moonfireCat(var.maxMoonfireCatTicksGainUnit) and bt.triggers == 2 and not bt.moonfireCat then
    ...
    if cast.able.thrashCat(var.thrashCatTicksGainUnit) and bt.triggers == 2 and not bt.thrashCat and not talent.thrashingClaws and bt.need and (debuff.thrashCat.refresh(var.thrashCatTicksGainUnit) or talent.brutalSlash)
    then
        if cast.thrashCat(var.thrashCatTicksGainUnit) then
...
---@param from? string
actionList.bloodtalons = function(from)
    ...
    if cast.able.prowl() and var.ui.prowlMode and cast.able.rake() and unit.gcd == 0 and not buff.suddenAmbush.exists() and (debuff.rake.refresh(var.maxRakeTicksGainUnit) or debuff.rake.pmultiplier(var.maxRakeTicksGainUnit) < 1.4) and not isStealthed() and not buff.apexPredatorsCraving.exists() then
        if cast.incarnProwl("player") or cast.prowl("player") then
    ...
    if cast.able.shadowmeld() and cast.able.rake() and not buff.suddenAmbush.exists() and (debuff.rake.refresh(var.maxRakeTicksGainUnit) or debuff.rake.pmultiplier(var.maxRakeTicksGainUnit) < 1.4) and not isStealthed() and not buff.apexPredatorsCraving.exists() then
        if cast.shadowmeld() then
    ...
    if cast.able.shred(var.maxTTDUnit) and not bt.shred and buff.clearcasting.react() and var.swipeTargets == 1 then
        if cast.shred(var.maxTTDUnit) then
    ...
    if cast.able.thrashCat(var.thrashCatTicksGainUnit) and debuff.thrashCat.refresh(var.thrashCatTicksGainUnit) and not bt.thrashCat and buff.clearcasting.react() and var.swipeTargets == 1 and not talent.thrashingClaws then
        if cast.thrashCat(var.thrashCatTicksGainUnit) then
    ...
    if talent.lunarInspiration and cast.able.moonfireCat(var.units.dyn40AOE) and not bt.moonfireCat and var.swipeTargets == 1 then
        if cast.moonfireCat(var.units.dyn40AOE) then
    ...
    if cast.able.thrashCat(var.thrashCatTicksGainUnit) and debuff.thrashCat.refresh(var.thrashCatTicksGainUnit) and not bt.thrashCat and not talent.thrashingClaws then
    ...
    if cast.able.shred(var.maxTTDUnit) and not bt.shred and var.swipeTargets == 1 and (not talent.wildSlashes or (not debuff.direFixation.exists(var.maxTTDUnit) and talent.direFixation)) then
        if cast.shred(var.maxTTDUnit) then
    ...
    if talent.lunarInspiration and cast.able.moonfireCat(var.maxMoonfireCatTicksGainUnit) and not bt.moonfireCat and var.swipeTargets < 5 then
        if cast.moonfireCat(var.maxMoonfireCatTicksGainUnit) then
    ...
    if talent.lunarInspiration and cast.able.moonfireCat(var.maxMoonfireCatTicksGainUnit) and not bt.moonfireCat then
        if cast.moonfireCat(var.maxMoonfireCatTicksGainUnit) then
    ...
    if cast.able.shred(var.maxTTDUnit) and (var.swipeTargets > 5 or talent.direFixation) and not bt.shred and not buff.suddenAmbush.exists() and not talent.wildSlashes then
        if cast.shred(var.maxTTDUnit) then
    ...
    if cast.able.thrashCat(var.thrashCatTicksGainUnit) and not bt.thrashCat then
        if cast.thrashCat(var.thrashCatTicksGainUnit) then
    ...
    if cast.able.rake(var.maxRakeTicksGainUnit) and not bt.rake and (var.swipeTargets > 4 and not talent.direFixation or talent.wildSlashes) then
        if cast.rake(var.maxRakeTicksGainUnit) then
...
---@param from? string
actionList.builder = function(from)
    ...
    if buff.clearcasting.react() then
        ...
        if actionList.clearcasting('builder') then
            return true
        end
    end
...
---@param from? string
actionList.clearcasting = function(from)
...
---@param from? string
actionList.finisher = function(from)
...
---@param from? string
actionList.variables = function(from)
...
local updateUiOptions = function()
    var.ui = {
        ...
        berserkBiteweave = ui.checked("Berserk Biteweave")
    }
end
...
local function runRotation()
    ...
    elseif (unit.inCombat() and var.profileStop) or ui.pause() or ui.mode.rotation == 4 then
        ...
        if unit.inCombat() then
            if actionList.def('run rotation') then return true end
        end
    end
end
local id = 103
